...
function parseProgress(text: string): ProgressInfo {
    ...
    for (const line of lines) {
        ...
        if (/^(info|warning|error|from|in file included from)/i.test(line) || /^\d+\s+warning(s)?\s+generated\./.test(line)) {
            logs.push(line);
    ...
    runningActions.sort((a,b) => parseInt(b.duration, 10) - parseInt(a.duration, 10));
...
export class LiveBepAnalyzer extends StaticBepAnalyzer {
    ...
    public tailFile(filePath: string): Promise<void> {
        ...
        if (!fs.existsSync(filePath)) {
            console.log(chalk.yellow(`Waiting for file to be created: ${filePath}...`));
        }
        ...
        if (id.buildStarted || id.started) {
            ...
            if (lastAction) {
                if (lastAction.success) {
                    const logLine = `${chalk.green('✔')} ${lastAction.label}`;
                    this.recentLogs.push(logLine);
                } else if (this.actionDetails !== 'none') {
                    const errorHeader = chalk.red.bold(`❌ FAILED: ${lastAction.mnemonic} ${lastAction.label}`);
                    this.recentLogs.push(errorHeader);
                    if (lastAction.stderrContent) {
                        const stderrLines = lastAction.stderrContent.trim().split('\n').slice(0, 3);
                        stderrLines.forEach(line => this.recentLogs.push(chalk.red(`  ${line}`)));
                        if (lastAction.stderrContent.trim().split('\n').length > 3) {
                           this.recentLogs.push(chalk.red('  ... (Full stderr in final report)'));
                        }
                    }
                }
            ...
            parsed.logs.forEach(log => {
                if (!this.recentLogs.some(existing => stripAnsi(existing) === stripAnsi(log))) {
    ...
    private renderLiveUpdate(): void {
        ...
        if(this.isFinished && this.buildFinished && this.buildStarted){
             status = this.buildFinished.overallSuccess ? chalk.green('SUCCESS') : chalk.red('FAILURE');
             const duration = parseInt(this.buildFinished.finishTimeMillis, 10) - parseInt(this.buildStarted.startTimeMillis, 10);
             totalTime = formatDuration(duration);
        }

        const statusLine = this.isFinished ? `Status: ${status}` : `Status: ${status} ${chalk.cyan(currentSpinner)}`;
        ...
        if (!this.isFinished) {
            if (progressInfo.total > 0) {
                const percent = Math.floor((progressInfo.completed / progressInfo.total) * 100);
                const bar = renderProgressBar(percent, 30);
                output.push(chalk.bold(`Overall Progress: [${bar}] ${percent}% (${progressInfo.completed.toLocaleString()}/${progressInfo.total.toLocaleString()})`));
            }

            if (progressInfo.runningActions.length > 0) {
                output.push('');
                output.push(chalk.bold.cyan('--- Running Actions ---'));
                const table = new Table({ colWidths: [70, 10], style: { 'head': [], 'border': [], 'padding-left': 0, 'padding-right': 0 } });
