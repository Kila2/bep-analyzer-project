...
export class TerminalReporter {
    ...
    public printReport(): void {
        const { 
            buildStarted, buildFinished, buildMetrics, buildToolLogs, actions, testSummaries,
            problems, failedTargets, workspaceStatus, configurations, optionsParsed,
            structuredCommandLine, buildPatterns, resolvedOutputs, convenienceSymlinks, actionDetails 
        } = this.data;
        ...
        const exitCodeName = buildFinished.exitCode?.name || (success ? this.t.t('buildSummary.exitCodeSuccess') : this.t.t('buildSummary.exitCodeFailure'));
        ...
        console.log(`${this.t.t('buildSummary.totalTime')}: ${chalk.yellow(formatDuration(finishTime - startTime))}`);
        if (buildMetrics?.timingMetrics) {
            const metrics = buildMetrics.timingMetrics;
            console.log(`  - ${this.t.t('buildSummary.analysisPhase')}: ${chalk.magenta(formatDuration(Number(metrics.analysisPhaseTimeInMs)))}`);
            console.log(`  - ${this.t.t('buildSummary.executionPhase')}: ${chalk.magenta(formatDuration(Number(metrics.executionPhaseTimeInMs)))}`);
        }

        // --- Build Environment & Options ---
        console.log(chalk.bold.cyan(`\n--- ${this.t.t('buildEnv.title')} ---`));
        const envTable = new Table({ colWidths: [30, 70], style: { border: ['gray'], head: [] }, wordWrap: true });
        envTable.push([{ colSpan: 2, content: chalk.bold.white(this.t.t('buildEnv.invocationDetails')) }]);
        envTable.push([this.t.t('buildEnv.command'), chalk.gray(buildStarted.command)]);
        if (buildPatterns.length > 0) {
             envTable.push([this.t.t('buildEnv.targets'), chalk.gray(buildPatterns.join(', '))]);
        }
        if (workspaceStatus) {
        ...
        if (optionsParsed?.explicitCmdLine && optionsParsed.explicitCmdLine.length > 0) {
            console.log(chalk.bold.white(`\n${this.t.t('buildEnv.explicitOptions')}:`));
            optionsParsed.explicitCmdLine.forEach(opt => console.log(chalk.gray(`  ${opt}`)));
        }
        
        if (structuredCommandLine) {
            console.log(chalk.bold.white(`\n${this.t.t('buildEnv.canonicalCommandLine')}:`));
            const cmd = structuredCommandLine.sections.flatMap(s => s.chunkList?.chunk || s.optionList?.option.map(o => o.combinedForm) || []).join(' ');
        ...
        if (buildMetrics) {
            ...
            perfTable.push(
                [{ colSpan: 2, content: chalk.bold.white(this.t.t('performanceMetrics.executionCaching')) }],
                [this.t.t('performanceMetrics.actionsCreated'), chalk.blue(formatNumber(metrics.actionSummary.actionsCreated || 'N/A'))],
                [this.t.t('performanceMetrics.actionsExecuted'), chalk.blue(formatNumber(metrics.actionSummary.actionsExecuted))]
            );
            if (metrics.actionSummary.actionCacheStatistics) {
                ...
                perfTable.push([this.t.t('performanceMetrics.actionCache'), `${hitRateColor(hitRate + '%')} ${this.t.t('performanceMetrics.actionCacheHit')} (${formatNumber(hits)} ${hitsLabel} / ${formatNumber(misses)} ${missesLabel})`]);
                ...
                if (stats.missDetails && stats.missDetails.length > 0) {
                    ...
                    if (missRows.length > 0) {
                        perfTable.push([{ colSpan: 2, content: chalk.white(`  ${this.t.t('performanceMetrics.cacheMissBreakdown')}:`) }]);
            ...
            if (metrics.memoryMetrics) {
                ...
                if(metrics.memoryMetrics.peakPostGcHeapSize) {
                    memRows.push([this.t.t('performanceMetrics.peakHeap'), chalk.magenta(formatBytes(metrics.memoryMetrics.peakPostGcHeapSize))]);
                }
                if(metrics.memoryMetrics.usedHeapSizePostBuild) {
                    memRows.push([this.t.t('performanceMetrics.usedHeapPostBuild'), chalk.magenta(formatBytes(metrics.memoryMetrics.usedHeapSizePostBuild))]);
                }
                if (memRows.length > 0) {
                    perfTable.push([{ colSpan: 2, content: chalk.bold.white(this.t.t('performanceMetrics.memoryUsage')) }]);
                ...
                if (metrics.memoryMetrics.garbageMetrics && metrics.memoryMetrics.garbageMetrics.length > 0) {
                    ...
                    if (gcRows.length > 0) {
                        perfTable.push([{ colSpan: 2, content: chalk.bold.white(this.t.t('performanceMetrics.gcByType')) }]);
        ...
        if (buildMetrics?.artifactMetrics) {
            ...
            const { sourceArtifactsRead, outputArtifactsSeen, topLevelArtifacts } = buildMetrics.artifactMetrics;
            const artifactTable = new Table({ head: [this.t.t('artifactMetrics.metric'), this.t.t('artifactMetrics.count'), this.t.t('artifactMetrics.size')], style: { head: ['cyan'] }});
            artifactTable.push([this.t.t('artifactMetrics.sourceRead'), formatNumber(sourceArtifactsRead.count), formatBytes(sourceArtifactsRead.sizeInBytes)]);
            artifactTable.push([this.t.t('artifactMetrics.outputSeen'), formatNumber(outputArtifactsSeen.count), formatBytes(outputArtifactsSeen.sizeInBytes)]);
            if(topLevelArtifacts) artifactTable.push([this.t.t('artifactMetrics.topLevel'), formatNumber(topLevelArtifacts.count), formatBytes(topLevelArtifacts.sizeInBytes)]);
        ...
        if (buildMetrics?.buildGraphMetrics) {
            ...
            const { actionCount, outputArtifactCount, builtValues } = buildMetrics.buildGraphMetrics;
            const graphTable = new Table({ style: { border: ['gray'] }});
            graphTable.push(
                [this.t.t('buildGraphMetrics.totalActions'), chalk.blue(formatNumber(actionCount))],
                [this.t.t('buildGraphMetrics.totalOutputs'), chalk.blue(formatNumber(outputArtifactCount))]
            );
            console.log(graphTable.toString());
            if (builtValues && builtValues.length > 0) {
                console.log(chalk.bold.cyan(`\n--- ${this.t.t('buildGraphMetrics.topSkyFunctions')} ---`));
                const skyFunctionTable = new Table({ head: [this.t.t('buildGraphMetrics.skyFunction'), this.t.t('buildGraphMetrics.evalCount')], style: { head: ['cyan'] } });
                builtValues
                    .sort((a,b) => Number(b.count) - Number(a.count))
                    .slice(0, 10)
        ...
        if (buildMetrics) {
            const hasWorkerMetrics = buildMetrics.workerMetrics && buildMetrics.workerMetrics.length > 0;
            const hasNetworkMetrics = buildMetrics.networkMetrics && buildMetrics.networkMetrics.systemNetworkStats;
            if (hasWorkerMetrics || hasNetworkMetrics) {
                ...
                if (hasWorkerMetrics) {
                    const totalActions = buildMetrics.workerMetrics!.reduce((sum, w) => sum + Number(w.actionsExecuted), 0);
                    workerNetworkTable.push([this.t.t('workerNetworkMetrics.totalWorkerActions'), formatNumber(totalActions)]);
                }
                if (hasNetworkMetrics) {
                    const { bytesSent, bytesRecv } = buildMetrics.networkMetrics!.systemNetworkStats!;
                    workerNetworkTable.push([this.t.t('workerNetworkMetrics.networkTraffic'), `${this.t.t('workerNetworkMetrics.sent')}: ${formatBytes(bytesSent)}, ${this.t.t('workerNetworkMetrics.received')}: ${formatBytes(bytesRecv)}`]);
                }
                ...
                if(log.contents) {
                    try {
                        ...
                        if (log.name === 'critical path') {
                             console.log(chalk.yellow(`${this.t.t('buildToolLogs.criticalPathSummary')}:`));
        ...
        if (failedTargets.length > 0) {
            ...
            failedTargets.forEach(target => {
                 const config = target.configId ? configurations.get(target.configId) : null;
        ...
        if (actionDetails !== 'none') {
            const actionsToDetail = actionDetails === 'all' 
                ? actions 
                : actions.filter(a => !a.success);

            if (actionsToDetail.length > 0) {
                const title = actionDetails === 'all' ? this.t.t('actionDetails.titleAll') : this.t.t('actionDetails.titleFailed');
                console.log(chalk.bold.cyan(`\n--- ${title} ---`));
                ...
                groupedActions.forEach((actions, label) => {
                    ...
                    const statusText = anyFailed ? this.t.t('actionDetails.statusFailure') : this.t.t('actionDetails.statusSuccess');
                    const status = anyFailed ? chalk.red.bold(`❌ ${statusText}`) : chalk.green.bold(`✔ ${statusText}`);
                    const countLabel = actions.length > 1 ? this.t.t('actionDetails.actionCountPlural') : this.t.t('actionDetails.actionCount');
                    ...
                    actions.forEach((action, index) => {
                        ...
                        console.log(`  [${index + 1}] ${this.t.t('actionDetails.type')}: ${chalk.blue(action.mnemonic)} | ${this.t.t('actionDetails.duration')}: ${chalk.yellow(duration)}`);

                        if (action.primaryOutput?.uri) {
                            const outputPath = action.primaryOutput.uri.replace('file://', '');
                            console.log(chalk.yellow(`    ${this.t.t('actionDetails.primaryOutput')}:`));
                        ...
                        if (action.argv && action.argv.length > 0) {
                            ...
                            console.log(chalk.yellow(`    ${this.t.t('actionDetails.commandLine')}:`));
                            console.log(chalk.gray(`      ${displayedCommand}`));
                        }
                        if (action.stderrContent && action.stderrContent.trim()) {
                            console.log(chalk.yellow(`    ${this.t.t('actionDetails.stderr')}:`));
        ...
        if (testSummaries.length > 0) {
            ...
            const table = new Table({ head: [this.t.t('testSummary.target'), this.t.t('testSummary.status'), this.t.t('testSummary.total'), this.t.t('testSummary.passed'), this.t.t('testSummary.failed')], colWidths: [40, 15, 10, 10, 10] });
        ...
        if (actions.length > 0) {
            ...
            const table = new Table({ head: [this.t.t('slowestActions.duration'), this.t.t('slowestActions.actionType'), this.t.t('slowestActions.outputTarget')], colWidths });
        ...
        if (resolvedOutputs.size > 0) {
            ...
            const outputsTable = new Table({ head: [this.t.t('buildOutputs.target'), this.t.t('buildOutputs.files')], style: { head: ['cyan'] }, colWidths: [40, 60], wordWrap: true });
        ...
        if (convenienceSymlinks.length > 0) {
            ...
            const symlinksTable = new Table({ head: [this.t.t('convenienceSymlinks.path'), this.t.t('convenienceSymlinks.action'), this.t.t('convenienceSymlinks.target')], style: { head: ['cyan'] }});
